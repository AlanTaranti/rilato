...
def test_opml_parse():
    ...
    assert len(res) == 7
    assert FeedImportData("https://austinkleon.com/feed/", ["Art and Design"]) in res
    assert (
        FeedImportData("https://www.cjchilvers.com/blog?format=rss", ["Art and Design"])
        in res
    )
    assert FeedImportData("https://andrealedwell.com/?feed=rss2", ["Friends"]) in res
    assert (
        FeedImportData(
            "https://preservecompany.com/blogs/bruces-muses.atom", ["Friends"]
        )
        in res
    )
    assert FeedImportData("https://kelake.org/feed/", ["Friends", "Close"]) in res
    assert (
        FeedImportData("http://www.alphabettes.org/feed/", ["Letterpress and Type"])
        in res
    )
    assert FeedImportData("https://blog.silverorange.com/feed", []) in res

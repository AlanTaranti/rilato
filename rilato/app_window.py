...
class RilatoAppWindow(BaseWindow):
    def __init__(self, application):
        ...
        super().__init__(
            ...
            shortcuts=[
                AppShortcut(
                    "F10", lambda *_: self.leaflet.left_headerbar.menu_btn.popup()
                ),
                AppShortcut("<Control>r", self.feedman.refresh),
                AppShortcut(
                    "<Control>f",
                    lambda *_: self.leaflet.left_headerbar.search_btn.set_active(
                        not self.leaflet.left_headerbar.search_btn.get_active()
                    ),
                ),
                AppShortcut("<Control>j", self.leaflet.sidebar.select_next_article),
                AppShortcut("<Control>k", self.leaflet.sidebar.select_prev_article),
                AppShortcut("<Control>plus", self.leaflet.webview.key_zoom_in),
                AppShortcut("<Control>minus", self.leaflet.webview.key_zoom_out),
                AppShortcut("<Control>equal", self.leaflet.webview.key_zoom_reset),
                AppShortcut(
                    "<Control>t",
                    lambda *_: self.leaflet.filter_flap.set_reveal_flap(
    ...
    def present(self):
        ...
        self.set_default_size(
            self.confman.nconf.window_width, self.confman.nconf.window_height
        )

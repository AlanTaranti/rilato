...
from rilato.simple_avatar import SimpleAvatar


class FeedsViewAllListboxRow(Gtk.ListBoxRow):
    def __init__(self):
        super().__init__()
        self.label = Gtk.Label(label=_("All feeds"), margin_top=12, margin_bottom=12)
...
class FeedsViewListboxRow(Gtk.ListBoxRow):
    def __init__(self, feed, description=True, count=True):
        ...
        self.builder = Gtk.Builder.new_from_resource(
            "/org/gabmus/rilato/ui/manage_feeds_listbox_row.ui"
        )
        ...
        self.icon.set_image(self.feed.title, self.feed.favicon_path)
        ...
        if count:
            ...
            self.feed.bind_property(
                "unread-count", self.count_label, "label", GObject.BindingFlags.DEFAULT
            )
        else:
